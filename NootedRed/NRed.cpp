...
void NRed::probePhoenix()
{
    ...
    if (complete && checkKernelArgument("-NRedPhoenixSMUProbe")) {
        // Phoenix MP1 13.0.4 exposes the standard v1 SMU mailbox at IP
        // discovery segment 1 (0x16000): message C2PMSG_66, argument
        // C2PMSG_82, and response C2PMSG_90. Keep this first stage strictly
        // read-only. A DisallowGfxOff transaction was consumed by PMFW, but
        // left the passthrough device in a state that VFIO function reset did
        // not recover, so mutation requires more of the native init sequence.
        ...
        this->iGPU->setProperty("NRed,phoenix-smu-mailbox-ready",
                               initialResp == kSMUFWResponseSuccess);
        SYSLOG("NRed", "Phoenix SMU read-only probe: message=0x%08X argument=0x%08X response=0x%08X ready=%s",
               initialMsg, initialArg, initialResp,
